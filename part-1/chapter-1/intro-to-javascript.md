...
- **Examples:**
  - **CoffeeScript** - Shorter syntax for clearer, concise code. Ruby devs love this
  - **TypeScript** - _Strict Data Typing_ to simplify development and support complex systems. - **Microsoft**
  - **Flow** - Also adds _Data Typing_ in a different way. - **Facebook**
  - **Dart** - Standalone language that has it's own engine that runs in non-browser environments (like mobile apps), but can also be transpiled into JS. - **Google**
  - **Brython** - **Python** transpiler to JS that enables the writing of applications in pure Python without JS
  - **Kotlin** - Modern, concise, and safe programming language that can target the browser and Node
